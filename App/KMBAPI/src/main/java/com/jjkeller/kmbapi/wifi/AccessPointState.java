...
public class AccessPointState implements Comparable<AccessPointState>, Parcelable {
    ...
    public static String getWifiConfigurationSecurity(WifiConfiguration wifiConfig) {
        if (AccessPointStateCompat.isEap(wifiConfig)) {
            return EAP;
...
//        }
        AccessPointStateCompat.updateWifiConfigurationEapFields(this, config);
...
//                config.password.setValue(convertToQuotedString(mPassword));
            	AccessPointStateCompat.setEapPassword(config, mPassword);
            }
